...
const CustomTooltip = (props: any) => {
  ...
  if (active && payload && payload.length) {
    ...
    const waveHeight = payload.find((p: any) => p.dataKey === 'waveHeight')?.value
...
export const ForecastChart = ({ forecasts, locale }: ForecastChartProps) => {
  ...
  return (
    <div className="h-80 rounded-2xl border border-white/10 bg-ocean-800/60 p-4">
      <ResponsiveContainer width="100%" height="100%">
        <LineChart data={chartData} margin={{ top: 10, right: 5, left: 0, bottom: 0 }}>
          ...
          <YAxis 
            ...
            fontSize={10}
            width={40}
            label={{ value: 'Altura (m)', angle: -90, position: 'insideLeft', style: { fill: '#7dd3fc', fontSize: 10 }, offset: 5 }}
          />
          <YAxis 
            yAxisId="right"
            orientation="right"
            ...
            fontSize={10}
            width={40}
            label={{ value: 'Energía', angle: 90, position: 'insideRight', style: { fill: '#fbbf24', fontSize: 10 }, offset: 5 }}
          />
